#!/usr/bin/env python3
"""
macOS Nintendo controller HID smoke test.

This script intentionally does not import joyconrobotics. It tests the first
blocking layer directly: macOS Bluetooth HID -> hidapi -> Nintendo reports.
"""
...
SUPPORTED_PRODUCT_IDS = {JOYCON_L_PRODUCT_ID, JOYCON_R_PRODUCT_ID, PRO_CONTROLLER_PRODUCT_ID}
...
def enumerate_joycons() -> list[JoyConDeviceInfo]:
    devices: list[JoyConDeviceInfo] = []
...
def choose_device(devices: list[JoyConDeviceInfo], controller: str, side: str, index: int | None) -> JoyConDeviceInfo:
    ...
    if controller != "any":
        controller_devices = [device for device in devices if device.controller_type == controller]
        if not controller_devices:
            raise SystemExit(f"No {controller} controller found.")
        return controller_devices[0]

    side = side.lower()
    if side != "any":
        side_devices = [device for device in devices if device.controller_type == side]
        if not side_devices:
            raise SystemExit(f"No {side} controller found.")
        return side_devices[0]
...
class JoyConHID:
    def __init__(self, info: JoyConDeviceInfo):
        self.info = info
...
def parse_buttons(report: list[int], controller_type: str) -> list[str]:
    ...
    right_mapping = [
        ...
        ("SR_R", 0x10), ("SL_R", 0x20), ("R", 0x40), ("ZR", 0x80),
    ]
    left_mapping = [
        ("DOWN", 0x01), ("UP", 0x02), ("RIGHT", 0x04), ("LEFT", 0x08),
        ("SR_L", 0x10), ("SL_L", 0x20), ("L", 0x40), ("ZL", 0x80),
    ]
...
def run_test(controller: JoyConHID, duration: float, rate_hz: float) -> int:
    controller.initialize()

    print("\nReading input reports. Move the stick, press buttons, and rotate the Joy-Con.")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Test Nintendo controller HID access on macOS.")
    ...
    parser.add_argument("--list", action="store_true", help="Only list detected Joy-Cons.")
    args = parser.parse_args()

    devices = enumerate_joycons()
    ...
    if args.list:
        return 0
    if not devices:
        print("\nPair and connect the Joy-Con or Pro Controller in macOS Bluetooth settings, then rerun this script.")
        return 1
    ...
    try:
        ...
        print(f"\nHID error: {exc}")
        print("If macOS says the controller is connected but open fails, remove it from Bluetooth and pair again.")
        return 1
    finally:
        controller.close()
